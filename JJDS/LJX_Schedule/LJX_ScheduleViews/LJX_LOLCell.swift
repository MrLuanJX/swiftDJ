...
class LJX_LOLCell: UITableViewCell {
        ...
        if index == 1 {
            ...
            for (i,model) in modelArray.enumerated() {
                ...
                if index == i+2 {
                    ...
                    // 五杀
                    if model.left_team_five_kills?.isEmpty == nil {
                        self.thirdLabel.isHidden = false
                        self.thirdImg.isHidden = true
                        ...
                        self.thirdImg.sd_setImage(with: URL.init(string: String(model.left_team_five_kills ?? String()) == "1" ?  bigModel.left_logo ?? "" : bigModel.right_logo ?? ""), placeholderImage: UIImage.init())
                    }
                    // 一血
